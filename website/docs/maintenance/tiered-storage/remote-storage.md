...
and `table.log.local-ttl` (default is 1 day). A configured local TTL must be greater than zero
and, when `table.log.ttl` is positive, less than or equal to `table.log.ttl`. A local segment is
